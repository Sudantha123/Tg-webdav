...
func (t *Telegram) run(ctx context.Context) error {
	...
	d.OnNewMessage(func(_ context.Context,_ tg.Entities,u *tg.UpdateNewMessage) error {
		...
		// Ingestion is intentionally limited to private bot inbox messages. Channel posts and group messages are storage output/control traffic, not new uploads.
		if _, ok := m.PeerID.(*tg.PeerUser); !ok { return nil }
		return t.ingest(m)
	})
...
func(f *FS)OpenFile(ctx context.Context,n string,flag int,_ os.FileMode)(webdav.File,error){
 ...
 if e!=nil && (flag&(os.O_CREATE|os.O_WRONLY|os.O_RDWR))!=0 {
  ...
  return &uploadFile{fs:f,name:strings.TrimPrefix(clean(n), "/"+f.t.cfg.Folder+"/"),file:tf},nil
 }
